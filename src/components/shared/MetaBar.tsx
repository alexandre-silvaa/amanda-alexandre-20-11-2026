type MetaBarProps = {
  ...
  city?: string;
  dateLabel?: string;
  shouldNotShowEndBorder?: boolean;
};
...
export function MetaBar({
  ...
  children,
  shouldNotShowEndBorder = false,
}: MetaBarProps) {
  return (
    <section>
      <header
        className="flex flex-col items-center border-b-[#8d8b88] border-t-4 border-t-[#202020] py-2"
      ...
      {children && !shouldNotShowEndBorder ? (
        <footer
          className="flex flex-col items-center border-b-[#8d8b88] border-t border-t-[#202020] py-2 pt-1.5"
          aria-label="Linha editorial"
        >
